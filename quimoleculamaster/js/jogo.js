...
function image() {
									     ...
									     oImg.setAttribute('src', 'imgs/atomos/' + _id +'.png');
		...
		{			
			...
			var divAtomo = $('<div>').attr({'id': _id + '-' + cont,
									...
									.css({'top': posicao.top + 'px',
											...
											'background': 'url("imgs/atomos/' + _id +'.png")no-repeat center',
...
function estaNaAreaDeEdicao(_id) {
	...
	if(posicao.left > 200 && posicao.left < 600) 
	{
		if(posicao.top > 0 && posicao.top < 600) 			
			return true;
		else
			return false;
	}
...
function colocarNaGrade(_position) {
	...
	_position.left = parseInt(_position.left/50) * 50;
	_position.top = parseInt(_position.top/50) * 50;
